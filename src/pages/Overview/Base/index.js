...
import {
	...
	COMPONENT_SPACE_HORIZONTAL, COMPONENT_MARGIN,
	ROOT_WIDTH, NODE_TYPE_MAP
} from "./constants";
...
export default props => {
	const { value = {}, secondFloor = [], getChild, onChange, onClick, nodeTypeMap, noPlus = 10 } = props;
				...
				if (type === "relation") {
						...
						>
							{
								parentPath?.length === 1
									? <>
										<img className="img1" src={typeMap[data.nodeType].img} />
					...
					ele = (
						...
						>
							...
							{
								!data?.noChild &&
								(data?.nodeType !== noPlus || data?.nodeRoot) &&
